...
function getLocalChrome() {
    ...
    for (const dir of platformDirs) {
        const exePath = path.join(versionPath, dir, process.platform === "win32" ? "chrome.exe" : "chrome");
...
async function scrapeProfile(profileUrl) {
    const browser = await puppeteerExtra.launch({
        ...
        dumpio: true, // Chrome stderr to Node
        env: {TMPDIR: process.env.TMPDIR || "/home/mcstaging3/tmp"},
    });
...
if(!profileUrl){
    console.error("SCRAPER_ERR: No profile URL provided");
    process.exit(1);
}
...
scrapeProfile(profileUrl)
    ...
    .catch(err => {
        console.error("SCRAPER_ERR:", err.stack || err);
        process.exit(1);
    });
